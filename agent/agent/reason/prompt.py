...
effective_apr_net_hedge = earn_apr
                        ...
                        - round_trip_friction (~1.8%/yr drag: Bybit fees on swap in/out @ 0.1% spot taker + perp open/close @ 0.055% perp taker ≈ 0.31% per round trip, amortized over the hold)
```
...
- Per-product `apr_source` values (resolution order — `apr_history` wins, then `measured_yield`, then `estimate_apr`):
  - `apr_history` — mean effective APR from Bybit's `/v5/earn/apr-history` (FlexibleSaving / OnChain). Pool-level, subsidy-inclusive AND hourly-smoothed: it captures the same promo subsidies as `measured_yield` (e.g. USD1 estimateApr=0.83% but apr_history ~2.1%, peaks ~4.8% under "Hold USD1, Earn WLFI") but is available for EVERY product — position or not — and is immune to the small-position rounding noise that can make `measured_yield` spike spuriously (a tiny stake's sub-precision hourly credit annualizes into a fake multi-% APR). This is the preferred ground truth; trust it ahead of everything else.
    - `apr_history_points` — the daily apr-history SERIES (oldest → newest fractional) that the `apr_history` mean is computed from. Read the TRAJECTORY, not just the mean: a held product whose daily APR is sliding down across the series is decaying — plan an exit before it craters, even if the mean still looks fine. A flat or rising series is a hold/scale signal — exactly the kind of durable rate that justifies sitting through multiple cycles rather than churning. A single-day blip up (one high point, rest flat) is a transient promo, NOT a reason to chase it (the round-trip friction outlives the blip). Use the series to distinguish a SUSTAINED rate (act on it) from a one-off (ignore it). None ⇒ no series this cycle.
...
- **`stability_score` (0–100, precomputed) — PREFER STABLE, don't just chase top-APR.** A single number summarizing how DURABLE a pick's yield is: it blends APR steadiness (flatness of `apr_history_points`) with price calm (small `price_change_7d_pct`), so you don't have to eyeball those raw fields — higher = steadier, more reliably-realized return. Two things to know: (1) **the snapshot already RANKS products with a moderate tilt toward stability** (a volatile coin's rank is discounted up to ~40%), so a high `stability_score` pick appearing near the top is there on merit, not just headline APR; (2) **when two picks' net yields (`effective_apr_net_hedge` for non-stable, `effective_apr` for stable) are within ~15% of each other, take the HIGHER `stability_score` one** — a steady 22%-net at score 85 beats a jumpy 25%-net at score 35; the steadier pick's realized return is tighter and survives multiple cycles without churn. A large net-yield gap still wins (stability is a tilt, not a veto), and a top-APR pick with a LOW `stability_score` is a flag to size DOWN toward a probe, not to fill the cap. None ⇒ no stability signal this cycle (advance-Earn / no apr-history and no perp) — fall back to the raw `apr_history_points` / `price_change_*` judgment.
...
- **Opportunity sizing is a function of risk class, not raw APR.** Before sizing up on an exceptional yield, classify the pick:
  - **Controllable risk** — hedgeable non-stable with positive funding subsidy, deep-liquidity LM at low leverage, stable with measured (not just quoted) promo, fixed-term with a tight strike — AND a healthy `stability_score` (steady APR, calm price). Risk is bounded by mechanics, not by hope. → Size at the upper edge of the venue cap; padding with weaker alternatives in the same risk class is dilution. A pick whose net APR is top-tier but whose `stability_score` is LOW (volatile price / sliding APR) is NOT controllable-risk — it belongs in the speculative tier (probe-sized), regardless of headline.
