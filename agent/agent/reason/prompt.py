...
- **Liquid-stable budget gate — compute BEFORE picking any non-stable.** `total_equity_usd` is NOT all spendable: most of a small vault is usually already locked in `earn_positions`. New hedged non-stable picks are funded ONLY from liquid stables, and each one costs `pick_usd` (spot Buy) + `~1.05 × pick_usd` (perp margin) ≈ `2.05 × pick_usd` of USDC/USDT. So the HARD ceiling on the SUM of NEW non-stable picks is `liquid_budget = (wallet.liquid_usdc_usd + wallet.liquid_usdt_usd) / 2.05`. Compute it from the snapshot and write it in the thesis (`liquid budget: ($X + $Y)/2.05 = $Z`). If `$Z` is below the smallest candidate's perp `min_notional_usd`, you CANNOT open ANY new non-stable pick this cycle — don't try; the validator rejects the whole decision. When liquid stables are scarce relative to locked Earn, the correct move is to HOLD existing positions at their current weights (a no-op the validator accepts) plus at most a small stable-only rebalance. Forcing the reference allocation below against an empty liquid budget is the #1 cause of rejected cycles.
...
{{
  "schema_version": 1,
  "captured_at": "<UTC ISO>",
  "wallet": {{ "total_equity_usd": "...", "liquid_usdc_usd": "...", "liquid_usdt_usd": "...", "accounts": [...] }},  // liquid_* = spendable stables (UNIFIED+FUND); total_equity includes capital LOCKED in earn_positions and is NOT all deployable
