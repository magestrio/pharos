...
If a non-stable Earn pick (OnChain or Flex) can't be hedged (perp pair missing, `pick_usd < min_notional_usd`, or `funding_rate_7d_avg` below floor), DOWNSIZE or DROP that pick. Validator rejects whole decisions with un-hedgeable non-stable picks. When feasibility clears AND its `effective_apr_net_hedge` beats your other candidates, **take the pick** — auto-hedging makes it cheap to use.
...
- **LM picks must be UNLEVERAGED (`max_leverage=1`).** Leveraged LP on a volatile token is speculative directional risk with a liquidation tail — outside the controlled-risk mandate. The snapshot already DROPS leveraged LM pairs from the choice set, so anything you see is 1x; the validator rejects any leveraged LM pick outright. An unleveraged LM pick is bounded by the `bybit_lm` venue cap (30%) like any other. Each held LM position carries `liquidation_distance_pct` in `lm_positions` (signed fraction; smaller = closer to wipe-out); if `< 0.10` on any held position, redeem it this cycle via partial REDEEM_LM (removeRate) even if APR still looks attractive.

  **Worked sizing recipe — read this before writing any `bybit_lm` venue.** `pick.weight` is the share of the LM venue, NOT of the total book; the validator multiplies `venue × pick` to get the absolute book share. LM pairs are all UNLEVERAGED (1x — leveraged pairs are dropped from the snapshot and rejected by the validator), so the only cap is the 30% `bybit_lm` venue max: keep `bybit_lm.weight ≤ 0.30` and every `venue × pick ≤ 0.30`. Two equal picks at `bybit_lm.weight=0.30` give `0.30 × 0.5 = 0.15` of book each — fine. A single pick uses `pick.weight=1.0`, so `bybit_lm.weight` itself IS the book share.
